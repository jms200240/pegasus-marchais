...
import type { Dentiste } from '../lib/types'
import loicHensmans from '../assets/dentistes/loic-hensmans.png'

// Photo statique pour le dentiste historique (rang 1) — zéro latence.
// Les dentistes ajoutés ensuite (rang >= 2) n'ont pas de photo, juste un avatar initiales.
...
interface DentistePickerProps {
  onSelect: (nom: string) => void
  onClose: () => void
}

export default function DentistePicker({ onSelect, onClose }: DentistePickerProps) {
  ...
  async function fetchDentistes() {
    ...
    setDentistes((data as Dentiste[]) ?? [])
    setLoading(false)
  }

  useEffect(() => { fetchDentistes() }, [])
  ...
  async function handleAdd() {
    ...
    setSaveError(null)
    try {
      const nextRang = dentistes.length > 0 ? Math.max(...dentistes.map(d => d.rang)) + 1 : 1
    ...
    <div className="fixed inset-0 z-[70] flex justify-center bg-black/40">
      <div className="w-full max-w-[390px] bg-[#F6F2EC] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3 flex-shrink-0 border-b border-gray-200/60 bg-white/70 backdrop-blur-sm">
          <h2 className="text-base font-black text-gray-900">Dentiste présent</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-3 gap-3">
              {dentistes.map(dentiste => {
                const photo = STATIC_PHOTOS[dentiste.rang] ?? dentiste.photo_url
                return (
                  <button
                    key={dentiste.id}
                    type="button"
                    onClick={() => onSelect(dentiste.nom)}
                  ...
                  >
                    <div className="w-full aspect-square rounded-full overflow-hidden border-2 border-transparent group-hover:border-green-300 group-active:scale-95 transition-all bg-white shadow-xs flex items-center justify-center">
                      {photo ? (
                        <img src={photo} alt={dentiste.nom} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-sm font-black" style={{ color: '#2f6b3f' }}>
                          {getInitiales(dentiste.nom)}
                        </span>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40 px-6">
          <div className="bg-white rounded-xl p-5 w-full max-w-xs">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-bold text-gray-900">Nouveau dentiste</p>
