...
- `GenealogyTreeSheet` (nouveau, `src/lib/genealogyLayout.ts`) : arbre généalogique visuel natif — boîtes (nom / sexe / année de naissance-décès) + connecteurs SVG calculés dynamiquement depuis `horses`+`genealogy` (génération = profondeur de lignée via `pere_id`/`mere_id`). Colonnes placées par barycentre rangée par rangée (méthode à la Sugiyama : chaque cheval à la moyenne des colonnes de ses parents) plutôt que par simple parcours DFS, pour rapprocher les couples et limiter les branches qui se croisent visuellement. Règles de tracé : ligne verticale simple si un seul parent est dans la cavalerie, "T" horizontal (bus routé au milieu de l'intervalle entre rangées, pas sur le bord des boîtes) si les deux parents y sont et sont au même niveau, éventail ("rateau") si plusieurs enfants partagent la même ancre. Mode ciblé (`focusHorseId`) : restreint l'arbre au cheval sélectionné + ses ascendants/descendants directs (`collectBloodline`), sans les "beaux-parents". Accessible via bouton "Arbre généalogique" dans Chevaux.tsx (sous la Cavalerie active) et via "Voir dans l'arbre généalogique" depuis chaque Fiche cheval (mode ciblé)
- Correctif généalogie : Chataigne et Cassis sont bien filles de Pomme (le fichier xlsx source avait leurs cellules Père/Mère vides ; confirmé via les lignes de connexion de l'organigramme HTML fourni) — `genealogy.mere_id`/`mere_name` corrigés. Haschich n'a aucun descendant.
- Chevaux.tsx : "Cavalerie historique" repliée par défaut (dépliable), même présentation `HorseCard` que la Cavalerie active
- FicheCheval : ascendants (Père/Mère/Père de mère) ET nouvelle section "Descendance" (résolution inverse) sont cliquables vers la fiche correspondante quand elle existe. Sections Vaccins/Soins/Bobos masquées pour la cavalerie historique (`!horse.is_active`, pas de données) — Identité/Généalogie/lien vers l'arbre conservés pour tous
