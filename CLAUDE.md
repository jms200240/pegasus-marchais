...
- Saisie de facture — ligne de solde auto (`InvoiceLineDraft.isAutoTail`) : la ligne 1 démarre en mode TTC, préremplie avec le montant total de la facture (modifiable comme une ligne normale). Tant que la somme des lignes "fixées" (non auto-gérées) n'atteint pas le montant total, une ligne normale supplémentaire apparaît automatiquement en fin de liste, montant TTC prérempli = solde restant, libellé "· reste à ventiler" ; éditer le montant (HT/TTC/TVA/mode) d'une ligne de solde la transforme en ligne fixe et fait apparaître une nouvelle ligne de solde derrière elle si un écart subsiste (cascade) — éditer son libellé ou sa ventilation ne casse pas le suivi automatique. Bloque la validation en cas de dépassement du montant total ("Ventilé : X€ — dépasse le montant de la facture de Y€"), pas de blocage/verrouillage artificiel sinon (la ligne de solde reste une `InvoiceLineDraft` comme les autres, soumise aux mêmes règles de validation label+ventilation).
